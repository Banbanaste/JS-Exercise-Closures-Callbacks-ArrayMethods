...
function processLastItem(stringList, callback /* CODE HERE */) {
  /* CODE HERE */
  const lastElement = stringList[stringList.length - 1];
...
function processSum(numberList, callback /* CODE HERE */) {
  ...
  let totalSum = numberList.reduce(
...
function firstNamesAllCaps(runners /* CODE HERE */) {
  ...
  let runnerCapNames = [];
  runners.forEach(runner => {
    runnerCapNames.push(`${runner.first_name.toUpperCase()}`);
  });
  return runnerCapNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize /* CODE HERE */) {
  /* CODE HERE */
  let filterByTshirtSize = runners.filter(
    runner => runner.shirt_size == tShirtSize
  );
  return filterByTshirtSize;
}
...
function tallyUpDonations(runners /* CODE HERE */) {
  /* CODE HERE */
  let totalDonnations = runners.reduce((accumulator, current) => {
    return accumulator + current.donation;
  }, 0);
  return totalDonnations;
}
